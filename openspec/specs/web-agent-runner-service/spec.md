# web-agent-runner-service Specification

## Purpose

定义 `runners/web-agent/` 这个 Node 运行时进程的生命周期与协议处理契约。runner 在 LarkIslandApp（M4 实现）启动时被 spawn 一次，长驻在用户机器上，通过 Unix socket（`web-agent-bridge` spec 定义的协议）接收任务、驱动 headless Chromium 配合 UI-TARS Browser Operator + VLM 完成网页操作，事件流式回灌给 app 端。

涵盖：
- runner 启动顺序：先连 socket 完成 v2 握手、注册 `webAgentRunner` 角色，再 launch Chromium。socket 不可达即 fail-fast 退出。
- profile 解析：`runWebAgentTask.profileName` → 凭证。本里程碑只支持 `qwen-default`（DashScope OpenAI 兼容端点 + qwen3-vl-plus）；未知 profile 或缺 `DASHSCOPE_API_KEY` 立即发 `webAgentTaskFailed{kind: vlmError}` 而不启动 GUIAgent。
- 任务调度：单任务串行；忙碌时第二条 `runWebAgentTask` 立即拒绝（`webAgentTaskFailed{kind: pageError, message: "runner busy"}`），不打断当前任务。
- 步骤事件回灌：把 `@ui-tars/sdk` GUIAgent 的 `onData` 回调翻译成 `webAgentStepUpdate` envelope，stepIndex 单调递增、screenshot 异步落盘到约定路径、wire 上只携带绝对路径。
- 异常分类：所有失败按 `WebAgentFailureKind` 四档（`vlmTimeout` / `vlmError` / `pageError` / `cancelled`）回灌为 `webAgentTaskFailed`；包括 GUIAgent 内部 catch 后通过 onError 通知（即使 run() 没 throw）的"伪 completed"路径。
- socket 断线策略：runner 不自重连。EOF 即 cancel in-flight 任务、关闭 LocalBrowser、exit 1，重启交给 M4 RunnerSupervisor。

历史：本 capability 由 change `m3-runner-socket-service` 引入（2026-04-28 归档）。M4 灵动岛 UI 重写 + RunnerSupervisor 是直接消费方；M5 飞书 skill 在此基础上扩展 profile 解析（增加 Doubao / 自部署 endpoint）和 starting URL / system prompt 注入。
## Requirements
